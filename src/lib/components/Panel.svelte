<script lang="ts">
	...
	$: dashboardHeaderClass = isDashboardOpen ? 'w-full bg-[#219EBC] rounded-tr' : '';

	const panelBaseClass =
		'Panel flex flex-col shadow-xl box-border items-center backdrop-blur-sm border-white bg-white/90';
...
<div class="{panelBaseClass} {panelClass} {extraClass}">
	<!-- /* ------------------------------ PANEL HEADER ------------------------------ */ -->
	<div class="Panel_header flex items-center {dashboardHeaderClass}">
		{#if isClosable}
			<button on:click={toggleSidePanel} class={isOpen ? 'p-2 text-white hover:animate-spin' : ''}>
				<svelte:component this={iconComponent} strokeWidth={2} size={26} class="m-2" />
			</button>
